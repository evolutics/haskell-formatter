...
-}
module Language.Haskell.Formatter.Process.Formatter
       (Formatter(..), format) where
...
format ::
       Formatter ->
         ExactCode.ExactCode -> Result.Result ExactCode.ExactCode
format formatter exact
  = do commentable <- checkedAttachComments formatter exact
       locatableCommentable <- tryZipLocationsComments locatable
                                 commentable
       locatable' <- checkedFormatActualCode formatter
...
tryZipLocationsComments ::
                        Code.LocatableCode ->
                          Code.CommentableCode -> Result.Result Code.LocatableCommentableCode
...
checkedFormatActualCode formatter
  ...
  where message
          = "Formatting the actual code changed the code itself."
...
checkedFormatComments formatter
  = transformWithCheck transform assert message
